    ...
    public class HalconUtils
    {
        ...
        public static void FindModel(HalconWindow winControl, HObject image, ModelParameter modelPar, out HTuple resultRow, out HTuple resultColumn, out HTuple resultAngle, out HTuple resultScores)
        {
            ...
            catch (Exception)
            {
                ...
                {
                    resultRow = new HTuple(0);
                    resultColumn = new HTuple(0);
                    resultAngle = new HTuple(0);
                    resultScores = new HTuple(0);
                }
            }
        }
        ...
        /// <param name="hv_rectangle2Param">矩形参数[包含旋转矩形5个参数]</param>
        /// <param name="hv_CaliperNumber">卡尺数量</param>
        /// <param name="hv_singleCaliperWidth">单个卡尺宽度0≤ singleCaliperWidth ≤ 1000</param>
        /// <param name="hv_AmplitudeThreshold">阈值0 ≤ AmplitudeThreshold ≤ 255</param>
        /// <param name="hv_sigma">西格玛0.400000 ≤ sigma ≤ 100.000000</param>
        /// <param name="hv_Transition">方向 all, positive, negative</param>
        /// <param name="hv_Select">点选择 all, first, last</param>
        /// <param name="hv_Min_Score">寻找分数all, first, last</param>
        /// <param name="hv_ShowRegion">是否显示区域all, first, last</param>
        /// <param name="hv_params">找到的边参数</param>
        public static void Rectangle2_look_line(out HTuple hv_params, HObject ho_Image, HTuple hv_rectangle2Param, HTuple hv_CaliperNumber, HTuple hv_singleCaliperWidth, HTuple hv_AmplitudeThreshold, HTuple hv_sigma, string hv_Transition = "all", string hv_Select = "all", double hv_Min_Score = 0.8, bool hv_ShowRegion = true, HalconWindow winControl = null)
        {

            HObject ho_Rectangle, ho_Contours, ho_Contour = null;
            HObject ho_Cross = null;

            // Local control variables 

            HTuple hv_start_row = new HTuple(), hv_start_col = new HTuple();
            HTuple hv_end_row = new HTuple(), hv_end_col = new HTuple();
            HTuple hv_MetrologyHandle = new HTuple(), hv_Width = new HTuple();
            HTuple hv_Height = new HTuple(), hv_Index = new HTuple();
            HTuple hv_Row = new HTuple(), hv_Column = new HTuple();
            HTuple hv_RowBegin = new HTuple(), hv_ColumnBegin = new HTuple();
            HTuple hv_RowEnd = new HTuple(), hv_ColumnEnd = new HTuple();
            ...
            using (HDevDisposeHelper dh = new HDevDisposeHelper())
            {
                ...
                HOperatorSet.GenRectangle2(out ho_Rectangle, hv_rectangle2Param.TupleSelect(0), hv_rectangle2Param.TupleSelect(1), hv_rectangle2Param.TupleSelect(2), hv_rectangle2Param.TupleSelect(3), hv_rectangle2Param.TupleSelect(4));
            }

            hv_start_row.Dispose();
            using (HDevDisposeHelper dh = new HDevDisposeHelper())
            {
                hv_start_row = (hv_rectangle2Param.TupleSelect(
                    0)) - ((hv_rectangle2Param.TupleSelect(3)) * (((hv_rectangle2Param.TupleSelect(
                    2))).TupleSin()));
            }
            hv_start_col.Dispose();
            using (HDevDisposeHelper dh = new HDevDisposeHelper())
            {
                hv_start_col = (hv_rectangle2Param.TupleSelect(
                    1)) + ((hv_rectangle2Param.TupleSelect(3)) * (((hv_rectangle2Param.TupleSelect(
                    2))).TupleCos()));
            }
            hv_end_row.Dispose();
            using (HDevDisposeHelper dh = new HDevDisposeHelper())
            {
                hv_end_row = (hv_rectangle2Param.TupleSelect(
                    0)) + ((hv_rectangle2Param.TupleSelect(3)) * (((hv_rectangle2Param.TupleSelect(
                    2))).TupleSin()));
            }
            hv_end_col.Dispose();
            using (HDevDisposeHelper dh = new HDevDisposeHelper())
            {
                hv_end_col = (hv_rectangle2Param.TupleSelect(
                    1)) - ((hv_rectangle2Param.TupleSelect(3)) * (((hv_rectangle2Param.TupleSelect(
                    2))).TupleCos()));
            }
            ...
            if (hv_ShowRegion)
            {
                ...
                winControl?.Disp_Region(ho_Rectangle, "green", "margin");
            }
            ...
            {
                hv_Index.Dispose();
                HOperatorSet.AddMetrologyObjectGeneric(hv_MetrologyHandle, "line", ((((hv_start_row.TupleConcat(
                    hv_start_col))).TupleConcat(hv_end_row))).TupleConcat(hv_end_col), 20, 5,
                    1, 30, new HTuple(), new HTuple(), out hv_Index);
            }

            //all 全部    positive 黑到白(暗到亮)    'negative'白到黑(亮到暗)
            HOperatorSet.SetMetrologyObjectParam(hv_MetrologyHandle, "all", "measure_transition", new HTuple(hv_Transition));
            //指定所需度量区域的数量(卡尺数量  越小卡尺间隔越大)
            HOperatorSet.SetMetrologyObjectParam(hv_MetrologyHandle, "all", "num_measures", hv_CaliperNumber);
            //每个计量对象成功拟合实例的最大数量
            HOperatorSet.SetMetrologyObjectParam(hv_MetrologyHandle, "all", "num_instances", 50);
            //西格玛
            HOperatorSet.SetMetrologyObjectParam(hv_MetrologyHandle, "all", "measure_sigma", hv_sigma);
            //整个卡尺宽度（一半）
            using (HDevDisposeHelper dh = new HDevDisposeHelper())
            {
                HOperatorSet.SetMetrologyObjectParam(hv_MetrologyHandle, "all", "measure_length1", hv_rectangle2Param.TupleSelect(4));
            }
            //单个卡尺的测量宽度
            HOperatorSet.SetMetrologyObjectParam(hv_MetrologyHandle, "all", "measure_length2", hv_singleCaliperWidth);
            //阈值
            HOperatorSet.SetMetrologyObjectParam(hv_MetrologyHandle, "all", "measure_threshold", hv_AmplitudeThreshold);
            //参数指定要使用的插值类型   双立体
            HOperatorSet.SetMetrologyObjectParam(hv_MetrologyHandle, "all", "measure_interpolation", "bicubic");
            //'all' 全部  'first' 第一个点            last 最后一个点
            HOperatorSet.SetMetrologyObjectParam(hv_MetrologyHandle, "all", "measure_select", new HTuple(hv_Select));
            //找的的最小分数
            HOperatorSet.SetMetrologyObjectParam(hv_MetrologyHandle, "all", "min_score", new HTuple(hv_Min_Score));
            //开始找边
            ...
            if ((int)(new HTuple((new HTuple(hv_Row.TupleLength())).TupleGreater(0))) != 0)
            {
                ...
                if (hv_ShowRegion)
                {
                    ...
                    {
                        ho_Cross.Dispose();
                        HOperatorSet.GenCrossContourXld(out ho_Cross, hv_Row, hv_Column, 2, hv_rectangle2Param.TupleSelect(
                            2));
                    }

                    winControl?.Disp_Region(ho_Cross, "12", "margin");
                ...
                if (hv_params == null)
                    hv_params = new HTuple();
                hv_params[new HTuple(hv_params.TupleLength())] = hv_RowBegin;
                if (hv_params == null)
                    hv_params = new HTuple();
                hv_params[new HTuple(hv_params.TupleLength())] = hv_ColumnBegin;
                if (hv_params == null)
                    hv_params = new HTuple();
                hv_params[new HTuple(hv_params.TupleLength())] = hv_RowEnd;
                if (hv_params == null)
                    hv_params = new HTuple();
                hv_params[new HTuple(hv_params.TupleLength())] = hv_ColumnEnd;
            }
