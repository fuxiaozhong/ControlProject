...
namespace HYProject.Model
{
    public class Cameras
    {
        ...
        public void Camera_ImageProcessEvent(string cameraName, HalconDotNet.HObject ho_image)
        {
            ...
            if (AppParam.Instance.Form_Product_Set2_State)
            {
                Form_Product_Set2.Instance.Form_Camera_ImageProcessEvent(cameraName, ho_image);
                return;
            }
            //相机页面打开了
            if (AppParam.Instance.Form_Camera_State)
            {
                Form_Camera.Instance.Form_Camera_ImageProcessEvent(cameraName, ho_image);
                return;
            }

            Log.WriteRunLog(name + "接收到图像开始处理......");
            Stopwatch stopwatch = new Stopwatch();
            stopwatch.Start();
            Work.CameraWork(cameraName, ho_image);
            stopwatch.Stop();
            Log.WriteRunLog(name + "图像处理完成,耗时:" + stopwatch.ElapsedMilliseconds + "ms");
        }
    }
