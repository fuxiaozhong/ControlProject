...
namespace HYProject.Model
{
    public class Work
    {
        ...
        public static void TCPSocketServer_SocketReceiveMessage1(System.Net.Sockets.Socket client, string clientSocketIp, string message)
        {
            ...
            //{
            //    if (HYMessageBox.ShowWarn("收到运行指令是否运行", "运行提示") != System.Windows.Forms.DialogResult.OK)
            //    {
            //        return;
            //    }
            //}
            ...
            else if (message.StartsWith("&OBG"))
            {
                ...
                {
                    Log.WriteErrorLog("相机1 解析数据出错", ex);
                }
            ...
            else if (message.StartsWith("CCD3,1"))
            {
                message = message.Replace("\r\n", "");
                string[] messageSplit = message.Split(',');
                CalibrationData.Instance.Cam3_Standard1_Point.X = double.Parse(messageSplit[2]);
                CalibrationData.Instance.Cam3_Standard1_Point.Y = double.Parse(messageSplit[3]);
                CalibrationData.Instance.Cam3_Standard1_Point.U = double.Parse(messageSplit[4]);
                Serialization.Save(CalibrationData.Instance, "CalibrationData");
                Log.WriteRunLog("相机3-1 放料基准标定成功");
                AppParam.Instance.lightSource.StateCH3 = false;
            }
            else if (message.StartsWith("CCD3,2"))
            {
                message = message.Replace("\r\n", "");
                string[] messageSplit = message.Split(',');
                CalibrationData.Instance.Cam3_Standard2_Point.X = double.Parse(messageSplit[2]);
                CalibrationData.Instance.Cam3_Standard2_Point.Y = double.Parse(messageSplit[3]);
                CalibrationData.Instance.Cam3_Standard2_Point.U = double.Parse(messageSplit[4]);
                Serialization.Save(CalibrationData.Instance, "CalibrationData");
                Log.WriteRunLog("相机3-2 放料基准标定成功2");
                AppParam.Instance.lightSource.StateCH3 = false;
            }
            else if (message.StartsWith("CCD2,1"))
            {
                message = message.Replace("\r\n", "");
                string[] messageSplit = message.Split(',');
                CalibrationData.Instance.Cam2_Standard1_Point.X = double.Parse(messageSplit[2]);
                CalibrationData.Instance.Cam2_Standard1_Point.Y = double.Parse(messageSplit[3]);
                CalibrationData.Instance.Cam2_Standard1_Point.U = double.Parse(messageSplit[4]);
                Serialization.Save(CalibrationData.Instance, "CalibrationData");
                Log.WriteRunLog("相机2-1 放料基准标定成功");
                AppParam.Instance.lightSource.StateCH3 = false;
            }
            else if (message.StartsWith("CCD2,2"))
            {
                message = message.Replace("\r\n", "");
                string[] messageSplit = message.Split(',');
                CalibrationData.Instance.Cam2_Standard2_Point.X = double.Parse(messageSplit[2]);
                CalibrationData.Instance.Cam2_Standard2_Point.Y = double.Parse(messageSplit[3]);
                CalibrationData.Instance.Cam2_Standard2_Point.U = double.Parse(messageSplit[4]);
                Serialization.Save(CalibrationData.Instance, "CalibrationData");
                Log.WriteRunLog("相机2-2 放料基准标定成功2");
                AppParam.Instance.lightSource.StateCH3 = false;
            }
            else if (message.StartsWith("CCD1,1"))
            {
                message = message.Replace("\r\n", "");
                string[] messageSplit = message.Split(',');
                CalibrationData.Instance.Cam1_Standard1_Point.X = double.Parse(messageSplit[2]);
                CalibrationData.Instance.Cam1_Standard1_Point.Y = double.Parse(messageSplit[3]);
                CalibrationData.Instance.Cam1_Standard1_Point.U = double.Parse(messageSplit[4]);
                Serialization.Save(CalibrationData.Instance, "CalibrationData");
                AppParam.Instance.lightSource.StateCH3 = false;
                Log.WriteRunLog("相机1-1 放料基准标定成功");
            }
            else if (message.StartsWith("CCD1,2"))
            {
                message = message.Replace("\r\n", "");
                string[] messageSplit = message.Split(',');
                CalibrationData.Instance.Cam1_Standard2_Point.X = double.Parse(messageSplit[2]);
                CalibrationData.Instance.Cam1_Standard2_Point.Y = double.Parse(messageSplit[3]);
                CalibrationData.Instance.Cam1_Standard2_Point.U = double.Parse(messageSplit[4]);
                Serialization.Save(CalibrationData.Instance, "CalibrationData");
                Log.WriteRunLog("相机1-2 放料基准标定成功2");
                AppParam.Instance.lightSource.StateCH3 = false;
            }
        }
        ...
        internal static void TCPSocketServer_SocketReceiveMessage2(Socket client, string clientSocketIp, string message)
        {
            ...
            else if (message.StartsWith("&OBG"))
            {
                ...
                {
                    Log.WriteErrorLog("相机2 解析数据出错", ex);
                }
                Cameras.Instance["Cam2"].Soft_Trigger();
            }
            //动静态GRR
        ...
        internal static void TCPSocketServer_SocketReceiveMessage3(Socket client, string clientSocketIp, string message)
        {
            ...
            else if (message.StartsWith("&OBG"))
            {
                ...
                {
                    Log.WriteErrorLog("相机3 解析数据出错", ex);
                }
                Cameras.Instance["Cam3"].Soft_Trigger();
            }
            //动静态GRR
        ...
        public static bool Find_Point(HObject image, HalconWindow halconWindow, string cam, string index, out HTuple row, out HTuple column, out HTuple angle)
        {
            try
            {
                ...
                if (cam == "相机1")
                {
                    if (index == "1")
                    {
                        ...
                        HalconUtils.CaliperMeasure(halconWindow, AppParam.Instance.product.Cam1_Top1, image, out _, out Top);
                        HalconUtils.CaliperMeasure(halconWindow, AppParam.Instance.product.Cam1_Left1, image, out _, out Left);
                    }
                    else if (index == "2")
                    {

                        halconWindow.Disp_Image(image);
                        HalconUtils.CaliperMeasure(halconWindow, AppParam.Instance.product.Cam1_Left2, image, out _, out Left);
                        HalconUtils.CaliperMeasure(halconWindow, AppParam.Instance.product.Cam1_Top2, image, out _, out Top);
                    }
                }
                else if (cam == "相机2")
                {
                    ...
                    {
                        halconWindow.Disp_Image(image);
                        HalconUtils.CaliperMeasure(halconWindow, AppParam.Instance.product.Cam2_Left1, image, out _, out Left);
                        HalconUtils.CaliperMeasure(halconWindow, AppParam.Instance.product.Cam2_Top1, image, out _, out Top);
                    }
                    else if (index == "2")
                    {
                        halconWindow.Disp_Image(image);
                        HalconUtils.CaliperMeasure(halconWindow, AppParam.Instance.product.Cam2_Left2, image, out _, out Left);
                        HalconUtils.CaliperMeasure(halconWindow, AppParam.Instance.product.Cam2_Top2, image, out _, out Top);
                    }
                }
                else if (cam == "相机3")
                {
                    ...
                    {
                        halconWindow.Disp_Image(image);
                        HalconUtils.CaliperMeasure(halconWindow, AppParam.Instance.product.Cam3_Left1, image, out _, out Left);
                        HalconUtils.CaliperMeasure(halconWindow, AppParam.Instance.product.Cam3_Top1, image, out _, out Top);
                    }
                    else if (index == "2")
                    {
                        halconWindow.Disp_Image(image);
                        HalconUtils.CaliperMeasure(halconWindow, AppParam.Instance.product.Cam3_Left2, image, out _, out Left);
                        HalconUtils.CaliperMeasure(halconWindow, AppParam.Instance.product.Cam3_Top2, image, out _, out Top);
                    }
                }
                row = new HTuple(0.0);
                ...
                HOperatorSet.IntersectionLines(Left[0], Left[1], Left[2], Left[3], Top[2], Top[3], Top[0], Top[1], out row, out column, out _);
                HOperatorSet.LineOrientation(Top[2], Top[3], Top[0], Top[1], out Phi);
                angle = Phi.TupleDeg();
                halconWindow.Disp_Cross(row, column, 100, 0, "green");
                // halconWindow.Disp_Message("Row:" + row + "\nCol:" + column + "\nAngle:" + Phi.TupleDeg(), 16, 20, 20, "blue");
                return true;
