    ...
    public partial class HalconWindow : UserControl
    {
        ...
        public void Disp_Image(HObject image)
        {
            try
            {
                if (image == null)
                {
                    HalconWindowHandle.ClearWindow();
                    return;
                }
                //清空十字架
                Cross.Clear();
        ...
        /// <param name="image"></param>
        /// <returns></returns>
        public bool Save_Image(HObject image = null)
        {
        ...
        /// <param name="column"></param>
        /// <param name="colorName"></param>
        public void Disp_Message(string message, int fontSize, HTuple row, HTuple column, string colorName)
        {
        ...
        {
            Set_display_font(hv_WindowHandle, hv_Size, "mono", "false", "false");
        ...
        public Rectangle1 Draw_Rectangle1(string colorName)
        {
            ...
            if (draw)
            {
                MessageBox.Show("请先完成之前的操作后在继续操作.", "提示-平行矩形");
        ...
        public Rectangle2 Draw_Rectangle2(string colorName)
        {
            ...
            if (draw)
            {
                MessageBox.Show("请先完成之前的操作后在继续操作.", "提示-旋转矩形");
            ...
            }; hWindowControl1.ContextMenuStrip = contextMenu_Hal; draw = false;
            HOperatorSet.SetLineStyle(HalconWindowHandle, new HTuple());
            return rectangle2;
        }
        ...
        public Line Draw_Line(string colorName)
        {
            ...
            if (draw)
            {
                MessageBox.Show("请先完成之前的操作后在继续操作.", "提示-直线");
                return new Line();
            }
            draw = true;
        ...
        public Circle Draw_Circle(string colorName)
        {
            ...
            if (draw)
            {
                MessageBox.Show("请先完成之前的操作后在继续操作.", "提示-圆");
                return new Circle();
            }
            draw = true;
        ...
        public Ellipse Draw_Ellipse(string colorName)
        {
            ...
            if (draw)
            {
                MessageBox.Show("请先完成之前的操作后在继续操作.", "提示-椭圆");
                return new Ellipse();
            }
            draw = true;
